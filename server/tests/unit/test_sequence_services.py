...
def test_sequence_id():
    ...
    assert (
        get_ga4gh_sequence_id("refseq:NC_000001.11")
        == "ga4gh:SQ.Ya6Rs7DHhDeg7YaOSg1EoNi3U_nQ9SvO"
    )
    assert (
        get_ga4gh_sequence_id("refseq:NM_152263.3")
        == "ga4gh:SQ.ijXOSP3XSsuLWZhXQ7_TJ5JXu4RJO6VT"
    )

    # namespace not always necessary
    assert (
        get_ga4gh_sequence_id("NM_152263.3")
        == "ga4gh:SQ.ijXOSP3XSsuLWZhXQ7_TJ5JXu4RJO6VT"
    )

    assert (
        get_ga4gh_sequence_id("refseq:NM_002529.3")
        == "ga4gh:SQ.RSkww1aYmsMiWbNdNnOTnVDAM3ZWp1uA"
    )

    # require recognized namespace
    match = re.escape(
        "Unable to retrieve GA4GH sequence ID for refsq:NM_002529.3: "
    ...
    match = re.escape(
        "Unable to retrieve GA4GH sequence ID for refseq:NC_000001.19: "
