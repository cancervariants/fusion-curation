...
export const CausativeEvent: React.FC<Props> = () => {
  ...
  const [causativeEventType, setCausativeEventType] = useState(
    fusion.causative_event?.event_type !== undefined ? fusion.causative_event?.event_type : null
  );
  ...
  const handleCauseChange = (event) => {
    ...
    const causativeEvent = { ...fusion.causative_event, event_type: event.target.value };
  ...
  const handleEnterDescription = (event) => {
    ...
    const causativeEvent = { ...fusion.causative_event, event_description: event.target.value };
    setFusion({ ...fusion, causative_event: causativeEvent });
