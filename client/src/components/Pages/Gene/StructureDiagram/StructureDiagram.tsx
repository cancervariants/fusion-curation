...
export const StructureDiagram: React.FC = () => {
  ...
  const selectStructure = (structure: string[], index: number) => {
    if (active === index){
      // TODO: another good use of a "SelectedSuggestion" context that exists independently of the fusion context
      // basically, which index of the SuggestionContext array has been selected?
      // this will let us keep the selection set to active when returning to this component (currently it clears the active class)
  ...
  return (
      ...
      <Grid container justify = "center">
        {suggestions.map((suggestion, index) => {
          ...
          let structure = [];
          suggestion.transcript_components.forEach(comp => {
            structure.push(comp); 
          })
          ...
          return (
            ...
            >
              ...
              <div key={index} className="sub-structure">
                {structure.map((s) => (    
                <span className={s.component_type}>{s.shorthand} </span>  
                ))}
              </div>
              ...
              <div className="regel-row">
                {
                  regEls.map((el) => (
                    <span className="regel-square">{el}</span>
                  ))
                }  
              </div>  
